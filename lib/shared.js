...
// Las comillas del SVG son DOBLES a propósito: encodeURIComponent no las
// toca a las simples (') pero sí a las dobles ("->%22). Este data URI se
// incrusta después dentro de un atajo onerror="this.src='...'" en varios
// lugares — una comilla simple sin escapar ahí cortaría ese string de JS a
// la mitad y dejaría el propio respaldo roto.
...
function detectBrand(p) {
  ...
  const hit = KNOWN_BRANDS.find(([, re]) => re.test(nombre));
  if (hit) return hit[0];
