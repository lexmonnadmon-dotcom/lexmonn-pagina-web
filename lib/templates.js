...
(function (root, factory) {
  ...
  function renderGalleryThumbs(p) {
    ...
    const thumbsHtml = images
      .map(
        (url, idx) =>
          `<img class="product-modal-thumb${idx === 0 ? " active" : ""}" src="${escapeHtml(url)}" data-src="${escapeHtml(url)}" width="56" height="56" loading="lazy" decoding="async" alt="${escapeHtml(p.nombre)} ${idx + 1}">`
      )
      .join("");
