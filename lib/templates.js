...
(function (root, factory) {
    ...
    <div class="product-card" data-id="${escapeHtml(p.id)}">
      <a class="product-card-link" href="${href}" target="_blank" rel="noopener" aria-label="Ver ${escapeHtml(p.nombre)} (se abre en una pestaña nueva)">
        <div class="product-img-wrap">
          ...
          <img class="product-img" src="${escapeHtml(p.imagen || PLACEHOLDER_IMG)}" alt="${escapeHtml(p.nombre)}"
  ...
  // Carrusel "Ofertas de Aniversario": los productos que tienen Precio_Oferta en la
  ...
  function renderOffersSection(products) {
    ...
    <h2 id="offers-title" class="offers-title">Ofertas de Aniversario</h2>
  ...
  //
  // Las baldosas NO miden todas lo mismo, y es a propósito: seis rectángulos
  // iguales se leen como una tabla, mientras que una grande acompañada de
  // otras más pequeñas se lee como una portada. El tamaño de cada una lo
  // decide `style.css` por su posición (.cat-tile-1, -2, ...), no este
  // archivo, para poder recomponer el mosaico sin volver a generar el HTML.
  //
  ...
  function renderCategoryTiles(cats) {
    ...
    const tiles = cats
      .map((c, i) => {
        ...
        return `<a class="cat-tile cat-tile-${i + 1}" href="/categoria/${c.slug}.html" data-cat="${escapeHtml(c.name)}">
        <img class="cat-tile-img" src="${escapeHtml(c.image || PLACEHOLDER_IMG)}" alt=""
          loading="${i === 0 ? "eager" : "lazy"}" decoding="async" onerror="this.src='${PLACEHOLDER_IMG}'">
        <span class="cat-tile-scrim" aria-hidden="true"></span>
        <span class="cat-tile-body">
          <span class="cat-tile-text">
            <span class="cat-tile-tag">${escapeHtml(etiqueta)}</span>
            <span class="cat-tile-name">${escapeHtml(c.name)}</span>
            <span class="cat-tile-tagline">${escapeHtml(c.tagline || "")}</span>
          </span>
      ...
      .join("");
    return `<div class="cat-mosaic">${tiles}</div>`;
  }
  ...
  function renderGalleryThumbs(p) {
    ...
    const thumbsHtml = images
      .map(
        (url, idx) =>
          `<img class="product-modal-thumb${idx === 0 ? " active" : ""}" src="${escapeHtml(url)}" data-src="${escapeHtml(url)}" width="56" height="56" loading="lazy" decoding="async" alt="${escapeHtml(p.nombre)} ${idx + 1}" onerror="this.src='${PLACEHOLDER_IMG}'">`
      )
      .join("");
